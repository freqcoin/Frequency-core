...
*Setup instructions for a gitian build of Frequency Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Frequency
...
to the Frequency GitHub release page.
...
- [Building Frequency Core](#building-frequency-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/frequency-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/frequency-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/frequency-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/frequency-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/frequency-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/frequency-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@frequency.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/frequency-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/frequency-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/frequency-osx-*-build.assert
```
