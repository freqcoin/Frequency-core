...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::FRQ);
    ...
    if (settings.contains("nAnonymizeFrequencyAmount") || reset)
        SoftSetArg("-anonymizefrequencyamount", settings.value("nAnonymizeFrequencyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in frequency.cpp during shotdown to also remove the window positions
