...
#include "qt/frequency/send.h"
#include "qt/frequency/forms/ui_send.h"
#include "qt/frequency/addnewcontactdialog.h"
#include "qt/frequency/qtutils.h"
#include "qt/frequency/sendchangeaddressdialog.h"
#include "qt/frequency/optionbutton.h"
#include "qt/frequency/sendconfirmdialog.h"
#include "qt/frequency/myaddressrow.h"
#include "qt/frequency/guitransactionsutils.h"
...
SendWidget::SendWidget(FrequencyGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (FRQ)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Frequency address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 FRQ");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_FRQ : AskPassphraseDialog::Context::Send_zFRQ, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zFRQ is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zFRQ selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zFRQ transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZFRQ_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zFRQ require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zFRQ transaction
        ...
        emit message("zFRQ transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isFRQ){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zFRQ in your balance to select."));
        }
    }
}
...
void SendWidget::onFRQSelected(bool _isFRQ){
    isFRQ = _isFRQ;
    setCssProperty(coinIcon, _isFRQ ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
