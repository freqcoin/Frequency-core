...
#include "qt/frequency/forms/ui_dashboardwidget.h"
#include "qt/frequency/sendconfirmdialog.h"
#include "qt/frequency/txrow.h"
...
DashboardWidget::DashboardWidget(FrequencyGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of FRQ staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-piv");
    setCssProperty(ui->labelSquarezPiv, "square-chart-zFRQ");
    ...
    ui->labelAmountPiv->setText("0 FRQ");
    ...
    ui->btnHowTo->setText(tr("How to get FRQ"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZFRQ;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair FRQ, zFRQ
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            zFRQ = (pair.second != 0) ? pair.second / 100000000 : 0;
        ...
        chartData->valueszPiv.append(zFRQ);

        int max = std::max(piv, zFRQ);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("FRQ");
    set1 = new QBarSet("zFRQ");
    set0->setColor(QColor(3,58,43));
    set1->setColor(QColor(31,128,101));
