...
#include "qt/frequency/privacywidget.h"
#include "qt/frequency/forms/ui_privacywidget.h"
#include "qt/frequency/qtutils.h"
#include "guiutil.h"
#include "qt/frequency/denomgenerationdialog.h"
#include "qt/frequency/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(FrequencyGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zFRQ anonymizes your FRQ by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zFRQ or convert back to FRQ"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 FRQ ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zFRQ");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zFRQ");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zFRQ");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zFRQ");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zFRQ");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zFRQ");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zFRQ");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zFRQ");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zFRQ Movements"));
    ...
    // Only Convert to FRQ enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzFRQ->setTitleClassAndText("btn-title-grey", "Total 0 zFRQ");
    ui->btnTotalzFRQ->setSubTitleClassAndText("text-subtitle", "Show denominations of zFRQ owned.");
    ui->btnTotalzFRQ->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select FRQ outputs to mint into zFRQ.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zFRQ");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzFRQ, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zFRQ");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of FRQ to mint into zFRQ"));
    }else{
        btnText = tr("Convert back to FRQ");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zFRQ to convert back into FRQ"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzFRQ->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzFRQ->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zFRQ is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zFRQ, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zFRQ").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zFRQ minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zFRQ converted back to FRQ"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any FRQ to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFRQ </b>";
    ...
    ui->btnTotalzFRQ->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
