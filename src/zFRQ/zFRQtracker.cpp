// Copyright (c) 2018-2020 The Frequency developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zFRQ/deterministicmint.h>
#include "zFRQtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzFRQTracker::CzFRQTracker(std::string strWalletFile)
{
...
CzFRQTracker::~CzFRQTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFRQTracker::Init()
{
...
bool CzFRQTracker::Archive(CMintMeta& meta)
{
...
bool CzFRQTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFRQTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFRQTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFRQTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFRQTracker::GetSerialHashes()
{
...
CAmount CzFRQTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFRQTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFRQTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFRQTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFRQTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFRQTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFRQTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFRQTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFRQTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFRQTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFRQTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFRQWallet* zFRQWallet)
{
    bool iszFRQWalletInitialized = (NULL != zFRQWallet);
    CMintMeta meta;
    ...
    if (! iszFRQWalletInitialized)
        zFRQWallet = new CzFRQWallet(strWalletFile);
    meta.isSeedCorrect = zFRQWallet->CheckSeed(dMint);
    if (! iszFRQWalletInitialized)
        delete zFRQWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFRQTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFRQTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFRQTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFRQTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFRQTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFRQTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzFRQWallet* zFRQWallet = new CzFRQWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zFRQWallet);
        }
        delete zFRQWallet;
        LogPrint("zero", "%s: added %d dzFRQ from DB\n", __func__, listDeterministicDB.size());
    }
