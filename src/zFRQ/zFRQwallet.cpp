// Copyright (c) 2017-2019 The Frequency developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zFRQwallet.h"
#include "main.h"
#include "txdb.h"
...
CzFRQWallet::CzFRQWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zFRQ seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFRQSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFRQSeed_deprecated()) {
                    LogPrintf("%s: Updated zFRQ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zFRQ seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zFRQ wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFRQWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFRQCount(nCountLastUsed);
    else if (!walletdb.ReadZFRQCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFRQWallet::Lock()
{
    seedMaster = 0;
}

void CzFRQWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFRQWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFRQ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFRQWallet::LoadMintPoolFromDB()
{
...
void CzFRQWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFRQWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFRQWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zFRQTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFRQWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFRQ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zFRQTracker which also adds to database
    pwalletMain->zFRQTracker->Add(dMint, true);
...
void CzFRQWallet::SeedToZFRQ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFRQWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFRQWallet::GenerateDeterministicZFRQ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFRQWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFRQWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFRQWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
