...
// Copyright (c) 2016-2020 The Frequency developers
...
public:
    ...
    bool WriteZFRQSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZFRQSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZFRQSeed_deprecated(uint256& seed);
    bool EraseZFRQSeed();
    bool EraseZFRQSeed_deprecated();

    bool WriteZFRQCount(const uint32_t& nCount);
    bool ReadZFRQCount(uint32_t& nCount);
