...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop Frequency server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "Frequency server stopping";
}
...
/*
        ...
        {"zerocoin", "getzFRQseed", &getzFRQseed, false, false, true},
        {"zerocoin", "setzFRQseed", &setzFRQseed, false, false, true},
        {"zerocoin", "generatemintlist", &generatemintlist, false, false, true},
        {"zerocoin", "searchdzFRQ", &searchdzFRQ, false, false, true},
        {"zerocoin", "dzFRQstate", &dzFRQstate, false, false, true},
*/
#endif // ENABLE_WALLET
};
...
{
    return "> frequency-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(std::string methodname, std::string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:25455/\n";
}
