...
from test_framework.test_framework import FrequencyTestFramework
...
class ZerocoinSpendTest(FrequencyTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zFRQ_bal, piv_bal):
            zFRQ_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zFRQ_bal)
            piv_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], piv_bal)
            return zFRQ_bal, piv_bal
        ...
        zFRQ_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zFRQ_balance, 6666)
        ...
        zFRQ_balance, balance = check_balances(denom_0, zFRQ_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zFRQ_balance, balance = check_balances(denom_2, zFRQ_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zFRQ_balance, balance = check_balances(denom_3, zFRQ_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zFRQ",
        ...
        _, _ = check_balances(denom_1, zFRQ_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
